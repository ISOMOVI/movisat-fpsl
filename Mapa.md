...
| 12_Nginx.md | Nginx — instalação, proxy reverso, timeout por rota, sites-enabled | nginx, proxy, porta 8005, timeout, conf |
...
| 18_Testes.md | **Testes** — regressão de extração (45 asserções, 9 termos reais em `tests/fixtures/`) + perfis de acesso (21). Cada número travado é um bug que já aconteceu | teste, regressão, extração, fixtures, baseline, asserção |
