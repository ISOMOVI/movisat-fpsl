...
# ─────────────────────────────────────────────────────────────────────────────
# 🚨 ESTE ARQUIVO É UM CLONE DE `equipamentos.py`, E É DE PROPÓSITO.
#
# O critério está em `docs/fpsl/28_Operacoes.md`:
#     Se a regra muda entre a aba velha e a nova, CLONA.
#     Se é infraestrutura sem regra, REUSA.
#
# Ele carrega decisão -- marcadores, limiares, `liberar_recipiente` e a regra
# do modelo -- e a regra 9 mudou: a WESO manda no modelo, e quando não há
# equipamento o operador escolhe pelo de-para. Importar o original obrigaria a
# escolher entre quebrar as telas velhas e travar esta.
#
# ⚠️ NÃO IMPORTE `equipamentos.py` AQUI. Nem "só uma constante". As duas telas
# velhas saem na F7 e nada deste arquivo pode sair junto.
#
# O que foi ACRESCENTADO ao clone está no fim do arquivo, sob "extensões da aba
# Operações". O corpo acima é cópia fiel, de propósito: é a área mais testada
# do projeto e reescrevê-la perderia o que ela aprendeu apanhando.
# ─────────────────────────────────────────────────────────────────────────────
import asyncio
import logging
import time
...
# ── extensões da aba Operações ───────────────────────────────────────────────
#
# Só o que a aba nova precisa e o original não expõe. Nada aqui muda o
# comportamento do que veio acima.

def chave(placa: str) -> str:
    """A mesma normalização de `_chave`, com nome público.

    A montagem da aba precisa casar placa com os dicionários devolvidos por
    `dados_das_placas` e `buscar_recipientes`, e as duas usam esta chave. Expor
    evita que o router invente a própria normalização -- que é como
    `RFD 2447` e `RFD2447` viravam veículos diferentes.
    """
    return _chave(placa)


def serie_que_entra(perfil: dict, recipientes: dict, placa: str) -> str:
    """Série do equipamento que ENTRA, vinda do recipiente já conferido.

    Vazio nos perfis que não usam recipiente -- `str.format` ignora chave que o
    template não cita, então passar sempre é mais simples que ramificar.

    🚨 Sem recipiente confiável sai `NUMERO DE SERIE`, para o técnico escrever
    na instalação, e NÃO o marcador de "não localizada", que significa outra
    coisa: um diz "ainda não existe", o outro diz "existia e eu não achei".
    """
